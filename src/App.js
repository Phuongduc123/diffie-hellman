...
import { findPrimitive } from './helper/index';
...
Yup.addMethod(Yup.number, 'checkPrimeNumber', function (errorMessage) {
  return this.test(`test-prime-number`, errorMessage, function (value) {
    ...
    return checkPrimeNumber(value) || createError({ path, message: errorMessage });
  });
});

function Content() {
  const { values, submitForm, errors, touched, setFieldValue } = useFormikContext();

  useEffect(()=> {
    setFieldValue("g", findPrimitive(values.p))
  }, [values.p])

  return (
    <Form className="form">
      <div className="params-p">
        <div className="params-text">Nhập tham số p:</div>
        <Field type="number" name="p" />
        {errors.p && touched.p && <div className="error">{errors.p}</div>}
      </div>
      <div className="params-g">
        <div className="params-text">Nhập tham số g:</div>
        <Field type="number" name="g" />
        {errors.g && touched.g && <div className="error">{errors.g}</div>}
      </div>
      <div className="params-Xa">
        <div className="params-text">Nhập khóa riêng Xa:</div>
        <Field type="number" name="Xa" />
        {errors.Xa && touched.Xa && <div className="error">{errors.Xa}</div>}
      </div>
      <div className="params-Xa">
        <div className="params-text">Nhập khóa riêng Xb:</div>
  ...
  return (
    <div className="App">
      <div className="container">
        <div className="title">Diffie Hellman</div>
        <Formik initialValues={initialValues} validationSchema={validationSchema} onSubmit={submit}>
          <Content/>
        </Formik>
        <div className="result">
          <div className="result-text">Kết quả tính toán:</div>
